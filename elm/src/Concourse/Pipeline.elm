...
order teamName pipelineNames csrfToken =
    Http.toTask <|
        Http.request
            { method = "PUT"
            , url = Concourse.host ++ "/api/v1/teams/" ++ teamName ++ "/pipelines/ordering"
...
fetchPipeline { teamName, pipelineName } =
    Http.toTask <|
        Http.request
            { method = "GET"
            , url = Concourse.host ++ "/api/v1/teams/" ++ teamName ++ "/pipelines/" ++ pipelineName
            , expect = Http.expectJson Concourse.decodePipeline
            , timeout = Nothing
            , withCredentials = True
            , headers = []
            , body = Http.emptyBody
...
fetchPipelines =
    Http.toTask <|
        Http.request
            { method = "GET"
            , url = Concourse.host ++ "/api/v1/pipelines"
            , expect = Http.expectJson <| Json.Decode.list Concourse.decodePipeline
            , timeout = Nothing
            , withCredentials = True
            , headers = []
            , body = Http.emptyBody
...
putAction action teamName pipelineName csrfToken =
    Http.toTask <|
        Http.request
            { method = "PUT"
            , url = Concourse.host ++ "/api/v1/teams/" ++ teamName ++ "/pipelines/" ++ pipelineName ++ "/" ++ action
            , headers = [ Http.header Concourse.csrfTokenHeaderName csrfToken ]
            , expect = Http.expectStringResponse (always (Ok ()))
            , body = Http.emptyBody
            , timeout = Nothing
