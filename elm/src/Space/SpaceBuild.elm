...
type Page
    = BuildPage Int
    | SpaceJobBuildPage Concourse.SpaceJobBuildIdentifier


initJobBuildPage : Concourse.TeamName -> Concourse.PipelineName -> Concourse.JobName -> Concourse.JobCombinationID -> Concourse.BuildName -> Page
initJobBuildPage teamName pipelineName jobName jobCombinationID buildName =
    SpaceJobBuildPage
        { teamName = teamName
    ...
    if model.browsingIndex > 0 && page == model.page then
        ...
        in
            ...
            , case page of
                ...
                SpaceJobBuildPage jbi ->
                    fetchSpaceJobBuild newIndex jbi
            )
                ...
                case ( model.job, build.job ) of
                    ( Nothing, Just buildJob ) ->
                        Cmd.batch
                            ...
                            , fetchBuildHistory buildJob build.jobCombinationID Nothing
                            ]
...
handleHistoryFetched history model =
    let
        ...
        jobCombinationID =
            Maybe.withDefault 0 <| Maybe.map (.jobCombinationID << .build) currentBuild
    in
        case ( history.pagination.nextPage, mJob ) of
            ( Nothing, _ ) ->
                ( withBuilds, Cmd.none )

            ( Just page, Just job ) ->
                ( withBuilds, Cmd.batch [ fetchBuildHistory job jobCombinationID (Just page) ] )
...
fetchSpaceJobBuild : Int -> Concourse.SpaceJobBuildIdentifier -> Cmd Msg
fetchSpaceJobBuild browsingIndex jbi =
    Task.attempt (BuildFetched browsingIndex) <|
        Concourse.Build.fetchSpaceJobBuild jbi
...
fetchBuildHistory : Concourse.JobIdentifier -> Concourse.JobCombinationID -> Maybe Concourse.Pagination.Page -> Cmd Msg
fetchBuildHistory job jobCombinationID page =
    Task.attempt BuildHistoryFetched <|
        Concourse.Build.fetchSpaceJobBuilds job jobCombinationID page
