...
public/elm.js: $(ELM_FILES) $(ELM_TESTS_FILES)
	@ cd elm && \
	  elm make --warn --output ../public/elm.js --yes src/Main.elm --report=json | \
	  jq -r '.[] | [.file,.region.start.line,.region.start.column,.overview] | @csv' | \
	  sed 's/"//g' | \
	  sed 's/,/:/g' | \
	  sed 's/^\./\.\/elm/g'
...
bindata.go: index.html $(PUBLIC_FILES)
	...
	@ go fmt bindata/bindata.go > /dev/null
